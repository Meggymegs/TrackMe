<?php
	...
	if ($error) {
	    header("location:accountPicture.php?msg=fail");
	} else if ($isSpecial){
		header("location:accountPicture.php?msg=special");
	} else {
		...
		if (move_uploaded_file($image['tmp_name'], $TARGET_PATH)){
			$query = "UPDATE `users_table` SET `user_profile_pic`= ' assets/images/" .$image['name'] ."' WHERE user_id = $temp";
		}
		
		$result = mysqli_query($db_server, $query);
		echo mysql_error();
		header("location:accountPicture.php?msg=success");
	}
